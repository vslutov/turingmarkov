...
а вместо названия состояния указывается только его номер. Конечное
состояние обозначается символом `!`.

Для записи правил используется таблица. Каждая строка символизирует состояние.
Каждый столбец - обозреваемый символ из алфавита `A`. На пересечении столбца
и строки записана правая часть правила, сотоящая из трех элементов
`a[i1],d[j],q[k1]`, разделенных запятыми. Символ или состояние могут быть
пропущено - это означает, что они не меняются. Столбцы разделяются пробельными
символами. Строки - символом переноса строки.
